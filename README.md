...
(אפשר להריץ את זה גם ישירות מהדפדפן דרך [reqbin.com](https://reqbin.com) או כלי דומה אם אין לכם טרמינל בהישג יד — רק לוודא שבוחרים בקשת POST, לא GET.)

**אם הבקשה "נתקעת" ולא מחזירה כלום:** עד לתיקון הזה בקוד, קריאות ל-GREEN-API/Peach לא היו מוגבלות בזמן — אם אחד השירותים לא זמין/איטי, הבקשה הייתה נתקעת לנצח בלי הודעת שגיאה. עכשיו כל קריאה מוגבלת ל-20 שניות (`HTTP_TIMEOUT_MS`) ומחזירה שגיאה ברורה אם היא נתקעת. ודאו שה-Railway עשה redeploy אחרי המשיכה האחרונה מה-GitHub.

התשובה תיראה בערך כך:
...
## היסטוריה — כמה אנשי קשר עברו במערכת, לפי תאריך

חשוב להבין את הגבול כאן: **אין למערכת מסד נתונים אמיתי**, וגם ל-Peach לא חשפנו endpoint לרשימה/ספירה (רק חיפוש איש קשר בודד לפי מזהה). `data/groups-state.json` שומר רק את ה-snapshot הנוכחי (מי ידוע עכשיו בכל קבוצה), לא היסטוריה.

בשביל זה נוסף `data/events.log` — קובץ JSONL (שורה אחת לכל אירוע) שנכתב בכל פעם שאיש קשר נוצר/עודכן/נשאר ללא שינוי, ונשמר על אותו Volume כמו `groups-state.json`. שאילתה עליו:

```bash
curl "https://xxx.up.railway.app/admin/history?token=<אם הגדרתם WEBHOOK_SHARED_SECRET>"
# מסוננת לקבוצה מסוימת:
curl "https://xxx.up.railway.app/admin/history?group=הלכה יומית בכלכלה יהודית 1"
```
...
{
  "byDate": {
    "2026-09-06": { "created": 12, "updated": 521 },
    "2026-09-07": { "created": 3 }
  }
}
```

**מגבלה שחשוב לדעת:** זה סופר רק מרגע שהתכונה הזו נפרסה — אין דרך לשחזר רטרואקטיבית מה קרה לפני כן (כולל ריצת ה-baseline הראשונית של 533+4 שכבר בוצעה). אם צריך את המספר ההיסטורי המדויק של אותה ריצה, המקור היחיד הוא הלוגים הגולמיים ב-Railway (כל עוד עוד נשמרים שם לפי מדיניות השמירה של Railway).
